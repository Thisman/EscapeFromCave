...
public class PlayerArmyController : MonoBehaviour
{
    ...
    public bool TryAddUnits(IUnitDefinition def, int amount) => Army.TryAddUnits(def, amount);
    public int RemoveUnits(IUnitDefinition def, int amount) => Army.RemoveUnits(def, amount);
    public bool TrySplit(IUnitDefinition def, int amount) => Army.TrySplit(def, amount);
    public bool TryMerge(int fromIndex, int toIndex) => Army.TryMerge(fromIndex, toIndex);

    public int GetTotal(IUnitDefinition def) => Army.GetTotalUnits(def);
